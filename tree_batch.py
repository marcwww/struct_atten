...
def get_child_ixs(nodes, adj_mat, device):
    ...
    """
    # notice that the index for child_ixs is parent index
    child_ixs = {}
    ...
    for l in range(max(nodes.keys())):
        ...
        # notice that the row indices are for parents
        ids = [np.nonzero(adj_mat[n.id])[0] for n in nodes[l]]
        try:
            ixs = [torch.LongTensor([id_to_ix[id] for id in id_list]).to(device) for id_list in ids]
...
def offset_node_lists(node_lists):
    ...
    for list_ix in range(len(node_lists)):
        for node in node_lists[list_ix]:
            ...
            # if the parent_id == 0, the new parent id will be -1
            if node.text_ix:
...
class Forest:
    ...
    def __init__(self, trees, device):
        """Create a new Forest.
...
def sexpr_to_tree(sexpr, stoi, device):

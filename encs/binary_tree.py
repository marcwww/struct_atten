...
class BinaryTreeLSTM(nn.Module):
    ...
    def forward(self, input, mask, return_select_masks=True):
        max_depth = input.size(1)
        length = (mask.sum(dim=1)).int()
        length_mask = basic.sequence_mask(sequence_length=length, # in this mask, the values at indices less than length are all 1, else 0
